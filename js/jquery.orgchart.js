  ...
  OrgChart.prototype = {
    ...
    dropHandler: function (event) {
      ...
      if (!$dropZone.siblings('.nodes').length) { // if the drop zone is a leaf node
        ...
        if ($dropZone.children('.title').length) {
          $dropZone.children('.title').append('<br><i class="'+  this.$chart.data('options').parentNodeSymbol + '"></i>');
        }
      } else {
      ...
      if (opts.verticalLevel && level >= opts.verticalLevel) {
        if ((level + 1) > opts.verticalLevel && Number(flags.substr(2,1))) {
          $nodeDiv.append('<i class="toggleBtn oci"></i>')
            .children('.title').append('<br><i class="'+ opts.parentNodeSymbol + '"></i>&nbsp;<span>' + data.children.length + '</span>');
        }
      } else {
    ...
    addChildren: function ($node, data) {
      ...
      if (!$node.find('.symbol').length) {
        $node.children('.title').append('<br><i class="'+ this.options.parentNodeSymbol + '"></i>&nbsp;<span>' + data.children.length + '</span>');
      }
      if ($node.closest('.nodes.vertical').length) {
        if (!$node.children('.toggleBtn').length) {
